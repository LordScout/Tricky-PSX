...
static void Stage_ScrollCamera(void)
{
		...
		if (stage.stage_id == StageId_1_1 && stage.song_step >= 1540 && stage.song_step <= 1580)
		{
			stage.camera.x += RandomRange(FIXED_DEC(-5, 10), FIXED_DEC(5, 10));
			stage.camera.y += RandomRange(FIXED_DEC(-10, 100), FIXED_DEC(10, 100));
		}
...
void Stage_DrawTexCol(Gfx_Tex *tex, const RECT *src, const RECT_FIXED *dst, fixed_t zoom, u8 cr, u8 cg, u8 cb)
{
	...
	if (tex == &stage.tex_hud0 && ((stage.stage_id == StageId_1_1) && stage.song_step >= 1425))
		return;

	if (tex == &stage.tex_hud1 && ((stage.stage_id == StageId_1_1) && stage.song_step >= 1425))
		return;

	if (tex == &stage.tex_hud0 && ((stage.stage_id == StageId_1_2) && stage.song_step >= 2158))
		return;
...
static void Stage_DrawHealth(s16 health, u8 i, s8 ox)
{
	...
	s8 dying;
	if (ox < 0)
		dying = (health >= 18000) * 60;
	else
		dying = (health <= 2000) * 60;
	...
	RECT_FIXED dst = {
		hx + ox * FIXED_DEC(28,1) - FIXED_DEC(32,1),
